...
- Run `deno install` after cloning or changing dependencies. `nodeModulesDir` is
  `"manual"`, so `deno task check` / `dev` fail with "Could not find a matching
  package … in the node_modules directory" until `node_modules/` is populated.
...
- **KV keys are household-scoped**: `[collection, householdId, id]`, e.g.
  ...
  over prose there.) Repos (`database/*.repo.ts`) are static classes; never call
  `Deno.openKv()` outside `database/db.ts`.
...
- `deno task db:seed` is destructive and dev-only: resets seeded collections and
  logs everyone out.
...
- `web-push` must stay externalized in `vite.config.ts` (both `ssr.external` and
  the SSR Rollup `external`) — its CommonJS deps break when bundled. Expect to
  do the same for other server-side CJS npm packages.
- `Deno.cron` must stay at module scope in `main.ts`: Deno Deploy extracts cron
  definitions at deploy time.
- Deploys are automatic: push to `main` → GitHub Actions → Deno Deploy; PRs
  against `main` also run the build.
